...
seasonal_avg = df.groupby('month')['t2m_norm_adj'].mean()
...
plt.tight_layout()
plt.savefig("results/plot1_raw_average_seasonal_profile.png", dpi=300, bbox_inches='tight')
plt.close()

# 2. Average Diurnal Profile
df['hour'] = df['valid_time'].dt.hour
diurnal_avg = df.groupby('hour')['t2m_norm_adj'].mean()
...
plt.tight_layout()
plt.savefig("results/plot2_raw_average_diurnal_profile.png", dpi=300, bbox_inches='tight')
plt.close()
...
plt.plot(last_four_weeks['valid_time'], last_four_weeks['t2m_norm_adj'])
...
plt.tight_layout()
plt.savefig("results/plot3_raw_last_two_weeks_of_hourly_data.png", dpi=300, bbox_inches='tight')
plt.close()
...
plt.figure(figsize=(12, 5))
plt.plot(daily_avg.index, daily_avg['t2m_norm_adj'])
...
plt.tight_layout()
plt.savefig("results/plot4_raw_last_year_of_daily_averages.png", dpi=300, bbox_inches='tight')
plt.close()
...
hourly_corr = autocorrelation(df['t2m_norm_adj'], lag=1)
daily_df = df.resample('D', on='valid_time').mean() # First resample to DataFrame, then access 't2m'
daily_corr = autocorrelation(daily_df['t2m_norm_adj'], lag=1)

monthly_df = df.resample('M', on='valid_time').mean()
monthly_corr = autocorrelation(monthly_df['t2m_norm_adj'], lag=1)
...
plt.tight_layout()
plt.savefig("results/plot5_raw_autocorrelations.png", dpi=300, bbox_inches='tight')
plt.close()